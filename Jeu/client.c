...
enum { STATE_MAIN_MENU, STATE_IP_MENU, STATE_TUTORIAL, STATE_SETTINGS, STATE_GAME, STATE_SOLO_GAME };
...
// --- MOTEUR DE TEXTE VECTORIEL RESTAURÉ ET COMPLET ---
void draw_char(SDL_Renderer* r, char c, int x, int y) {
    ...
    switch(c) {
        ...
        case 'J':
            ...
            break;
        case 'K': // AJOUT CORRECTION TERMINALE
        ...
        case 'P':
            ...
            SDL_RenderDrawLine(r, x1, y0, x1, y1); SDL_RenderDrawLine(r, x0, y1, x1, y1);
            break;
        case 'Q': // AJOUT CORRECTION TERMINALE
            SDL_RenderDrawLine(r, x0, y0, x1, y0); SDL_RenderDrawLine(r, x1, y0, x1, y2);
            SDL_RenderDrawLine(r, x1, y2, x0, y2); SDL_RenderDrawLine(r, x0, y2, x0, y0);
...
// --- SYSTÈME DE COLLISION AVEC GLISSEMENT TACTIQUE ---
...
// --- RAYCASTING DE LIGNE DE VISÉE (FOG OF WAR / STEALTH) ---
    ...
    for (int i = 1; i < steps; i++) {
        ...
        for (int o = 0; o < num_obstacles; o++) {
            if (check_x >= obstacles[o].x && check_x <= obstacles[o].x + obstacles[o].w &&
                ...
                return 0; // Vue bloquée par une caisse !
            }
        }
    }
    return 1; // En ligne de mire !
}
...
int main(int argc, char* argv[]) {
    ...
    srand(time(NULL));
    for(int i=0; i<80; i++) { starX[i] = rand() % WINDOW_WIDTH; starY[i] = rand() % WINDOW_HEIGHT; }
    ...
    float solo_px = 100, solo_py = 300; // Position joueur local
    float last_dx = 1.0f, last_dy = 0.0f; // Direction du viseur
    ...
    while(running) {
        ...
        while(SDL_PollEvent(&event)) {
            ...
            if (appState == STATE_MAIN_MENU) {
                if (event.type == SDL_KEYDOWN) {
                    if (event.key.keysym.sym == SDLK_UP || event.key.keysym.sym == SDLK_z) menuSelected = (menuSelected - 1 + 4) % 4;
                    else if (event.key.keysym.sym == SDLK_DOWN || event.key.keysym.sym == SDLK_s) menuSelected = (menuSelected + 1) % 4;
                    else if (event.key.keysym.sym == SDLK_RETURN || event.key.keysym.sym == SDLK_KP_ENTER) {
                        if (menuSelected == 0) appState = STATE_IP_MENU;
                        else if (menuSelected == 1) { 
                            appState = STATE_SOLO_GAME; // Init mode solo
                ...
                if (event.type == SDL_TEXTINPUT) {
                    if (strlen(server_ip) < 25 && ((*event.text.text >= '0' && *event.text.text <= '9') || *event.text.text == '.')) strcat(server_ip, event.text.text);
                }
                else if (event.type == SDL_KEYDOWN) {
                    if (event.key.keysym.sym == SDLK_BACKSPACE && strlen(server_ip) > 0) server_ip[strlen(server_ip) - 1] = '\0';
                    else if (event.key.keysym.sym == SDLK_ESCAPE) appState = STATE_MAIN_MENU;
                    else if (event.key.keysym.sym == SDLK_RETURN || event.key.keysym.sym == SDLK_KP_ENTER) {
                        if(strlen(server_ip) > 0) { serverAddr.sin_addr.s_addr = inet_addr(server_ip); appState = STATE_GAME; }
                    }
                }
            }
            else if (appState == STATE_TUTORIAL || appState == STATE_SETTINGS) {
                if (event.type == SDL_KEYDOWN) { if (event.key.keysym.sym == SDLK_ESCAPE || event.key.keysym.sym == SDLK_RETURN) appState = STATE_MAIN_MENU; }
            }
            else if (appState == STATE_GAME || appState == STATE_SOLO_GAME) {
                if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE) appState = STATE_MAIN_MENU;
                
                // Tir mécanique Solo (8 directions nerveuses)
                if (appState == STATE_SOLO_GAME && !player_dead && event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_SPACE) {
                    for(int i=0; i<30; i++) {
                        if(!bullets[i].active) {
                            bullets[i].x = solo_px + PLAYER_SIZE/2; bullets[i].y = solo_py + PLAYER_SIZE/2;
                            bullets[i].dx = last_dx * 9.0f; bullets[i].dy = last_dy * 9.0f;
                            bullets[i].active = 1; bullets[i].from_bot = 0;
                            break;
                        }
                    }
                }
        ...
        // --- ARRIÈRE-PLAN ---
        SDL_SetRenderDrawColor(renderer, 8, 10, 18, 255); SDL_RenderClear(renderer);
        SDL_SetRenderDrawColor(renderer, 180, 180, 255, 120);
        for(int i=0; i<80; i++) {
            if (appState != STATE_GAME && appState != STATE_SOLO_GAME) starX[i] = (starX[i] - 1 + WINDOW_WIDTH) % WINDOW_WIDTH;
        ...
        if (appState == STATE_MAIN_MENU) {
            ...
            SDL_SetRenderDrawColor(renderer, 255, 60, 60, 255); draw_string(renderer, "LAN BLASTER", WINDOW_WIDTH/2 - 121, 80);
            SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255); draw_string(renderer, "TACTICAL PRO", WINDOW_WIDTH/2 - 121, 115);

            const char* choices[] = {"REJOINDRE MULTI", "MODE SOLO BOTS", "TUTORIEL", "PARAMETRES"};
            int offsets[] = {154, 143, 88, 110};
            for(int i=0; i<4; i++) {
                if(menuSelected == i) { SDL_SetRenderDrawColor(renderer, 255, 60, 60, 255); draw_char(renderer, '>', WINDOW_WIDTH/2 - offsets[i] - 40, 230 + i*50); }
        ...
        // --- RENDU : SAISIE IP ---
            ...
            SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255); draw_string(renderer, "ENTREZ L IP DU SERVEUR", WINDOW_WIDTH/2 - 242, 140);
            SDL_Rect ipBox = { WINDOW_WIDTH/2 - 240, WINDOW_HEIGHT/2 - 40, 480, 80 };
            SDL_SetRenderDrawColor(renderer, 15, 20, 30, 255); SDL_RenderFillRect(renderer, &ipBox);
            SDL_SetRenderDrawColor(renderer, 255, 60, 60, 255); SDL_RenderDrawRect(renderer, &ipBox);
            if(strlen(server_ip) > 0) draw_string(renderer, server_ip, (WINDOW_WIDTH / 2) - (strlen(server_ip)*22)/2, (WINDOW_HEIGHT / 2) - 11);
        }

        // --- RENDU : TUTORIEL CORRIGÉ (SANS OCTETS PARASITES) ---
        else if (appState == STATE_TUTORIAL) {
            SDL_SetRenderDrawColor(renderer, 255, 60, 60, 255); draw_string(renderer, "CONSIGNES TACTIQUES", WINDOW_WIDTH/2 - 209, 60);
            ...
            SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255); draw_string(renderer, "RETOUR : ENTREE OU ECHAP", WINDOW_WIDTH/2 - 264, 490);
        }

        // --- RENDU : PARAMÈTRES CORRIGÉS ---
        else if (appState == STATE_SETTINGS) {
            SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255); draw_string(renderer, "CONFIG COMPETITIVE", WINDOW_WIDTH/2 - 198, 80);
            SDL_SetRenderDrawColor(renderer, 120, 140, 160, 255);
            draw_string(renderer, "MODE NET  - NETWORK UDP LOCAL", 100, 200);
            draw_string(renderer, "TICKRATE  - 64 TICK RATE SYSTEM", 100, 260);
            draw_string(renderer, "COLLISION - ACTIVE ET IMPERMEABLE", 100, 320);
            SDL_SetRenderDrawColor(renderer, 255, 60, 60, 255); draw_string(renderer, "RETOUR : ENTREE", WINDOW_WIDTH/2 - 165, 480);
        }

        // --- DESSIN DES BLOCS/CAISSES DANS LES DEUX MODES DE JEU ---
        if (appState == STATE_GAME || appState == STATE_SOLO_GAME) {
            for(int o=0; o<num_obstacles; o++) {
                SDL_SetRenderDrawColor(renderer, 25, 28, 42, 255); SDL_RenderFillRect(renderer, &obstacles[o]);
                SDL_SetRenderDrawColor(renderer, 240, 90, 40, 255); SDL_RenderDrawRect(renderer, &obstacles[o]);
        ...
        // --- MODE DE JEU 1 : MULTIJOUEUR EN LIGNE ---
        if (appState == STATE_GAME) {
            ...
            while(recvfrom(sock, (char*)&latestState, sizeof(latestState), 0, (struct sockaddr*)&serverAddr, &len) > 0) gameState = latestState;
            ...
            for(int i=0; i<MAX_PLAYERS; i++) {
                if(gameState.players[i].connected) {
                    SDL_SetRenderDrawColor(renderer, (i==0)*255, 220, 255, 255);
                    SDL_Rect pRect = {gameState.players[i].x, gameState.players[i].y, PLAYER_SIZE, PLAYER_SIZE};
                    SDL_RenderFillRect(renderer, &pRect);
                }
            }
        }

        // --- MODE DE JEU 2 : SOLO TACTIQUE (KILL THE CROWS / STEALTH LOOK) ---
            ...
            if(!player_dead) {
                ...
                if (move_x != 0 || move_y != 0) {
                    ...
                    // Normalisation pour aligner le tir propre
                    float mag = sqrt(last_dx*last_dx + last_dy*last_dy);
                    last_dx /= mag; last_dy /= mag;
                }

                // Glissement contre les caisses (Axe Y)
                solo_py += move_y;
                if(check_obstacle_collision((int)solo_px, (int)solo_py, PLAYER_SIZE) || solo_py < 0 || solo_py > WINDOW_HEIGHT-PLAYER_SIZE) solo_py -= move_y;
                // Glissement contre les caisses (Axe X)
                solo_px += move_x;
                if(check_obstacle_collision((int)solo_px, (int)solo_py, PLAYER_SIZE) || solo_px < 0 || solo_px > WINDOW_WIDTH-PLAYER_SIZE) solo_px -= move_x;
            }

            // Gestion de l'intelligence artificielle et de la visibilité
            int active_bots = 0;
            for(int b=0; b<6; b++) {
                if(!bots[b].active) continue;
                active_bots++;

                // Patrouille basique des bots
                bots[b].y += 1.0f * bots[b].patrol_dir;
                if(check_obstacle_collision((int)bots[b].x, (int)bots[b].y, PLAYER_SIZE) || bots[b].y < 50 || bots[b].y > 530) {
                    bots[b].patrol_dir *= -1;
                }

                // ANALYSE DU CHAMP DE VISION INTERACTIF
                int visible = check_line_of_sight((int)solo_px + PLAYER_SIZE/2, (int)solo_py + PLAYER_SIZE/2, (int)bots[b].x + PLAYER_SIZE/2, (int)bots[b].y + PLAYER_SIZE/2);
                
                if (visible && !player_dead) {
                    // Le bot te voit ! Dessin d'un réticule d'alerte rouge sur le joueur
                    SDL_SetRenderDrawColor(renderer, 255, 0, 0, 200);
                    SDL_RenderDrawLine(renderer, bots[b].x+16, bots[b].y+16, solo_px+16, solo_py+16);

                    bots[b].cooldown--;
                    if(bots[b].cooldown <= 0) {
                        // Tir réflexe ultra-nerveux vers ta position
                        for(int i=0; i<30; i++) {
                            if(!bullets[i].active) {
                                bullets[i].x = bots[b].x + PLAYER_SIZE/2; bullets[i].y = bots[b].y + PLAYER_SIZE/2;
                                float b_dx = (solo_px + PLAYER_SIZE/2) - bullets[i].x;
                                float b_dy = (solo_py + PLAYER_SIZE/2) - bullets[i].y;
                                float mag = sqrt(b_dx*b_dx + b_dy*b_dy);
                                bullets[i].dx = (b_dx / mag) * 6.5f; bullets[i].dy = (b_dy / mag) * 6.5f;
                                bullets[i].active = 1; bullets[i].from_bot = 1;
                                break;
                            }
                        }
                        bots[b].cooldown = 70; // reset
                    }

                    // DESSIN DU BOT (Seulement s'il est visible / pas planqué !)
            ...
            // Gestion tactique des projectiles
            for(int i=0; i<30; i++) {
                if(!bullets[i].active) continue;
                bullets[i].x += bullets[i].dx; bullets[i].y += bullets[i].dy;

                // Collision balle contre les caisses solides
                if(check_obstacle_collision((int)bullets[i].x, (int)bullets[i].y, 6)) { bullets[i].active = 0; continue; }

                if(bullets[i].from_bot) {
                    // Un bot touche le joueur (One-shot fatal !)
                    if(!player_dead && bullets[i].x >= solo_px && bullets[i].x <= solo_px+PLAYER_SIZE && bullets[i].y >= solo_py && bullets[i].y <= solo_py+PLAYER_SIZE) {
                        player_dead = 1; bullets[i].active = 0;
                    }
                    SDL_SetRenderDrawColor(renderer, 255, 120, 0, 255); // Balles ennemies orange
                } else {
                    // Le joueur touche un bot visible
                    for(int b=0; b<6; b++) {
                        if(bots[b].active && bullets[i].x >= bots[b].x && bullets[i].x <= bots[b].x+PLAYER_SIZE && bullets[i].y >= bots[b].y && bullets[i].y <= bots[b].y+PLAYER_SIZE) {
                            bots[b].active = 0; bullets[i].active = 0; solo_kills++;
                        }
                    }
                    SDL_SetRenderDrawColor(renderer, 0, 255, 255, 255); // Balles joueur cyan
                }
                SDL_Rect bulRect = {(int)bullets[i].x - 3, (int)bullets[i].y - 3, 6, 6};
                SDL_RenderFillRect(renderer, &bulRect);
            }

            // Rendu graphique du joueur local
            if(!player_dead) {
                SDL_SetRenderDrawColor(renderer, 50, 255, 120, 255);
                SDL_Rect pRect = {(int)solo_px, (int)solo_py, PLAYER_SIZE, PLAYER_SIZE};
                SDL_RenderFillRect(renderer, &pRect);

                // Dessin dynamique du viseur laser autour du joueur (Croix directionnelle)
                SDL_SetRenderDrawColor(renderer, 255, 255, 255, 150);
                int cx = solo_px + PLAYER_SIZE/2 + last_dx * 25;
                int cy = solo_py + PLAYER_SIZE/2 + last_dy * 25;
                SDL_RenderDrawLine(renderer, cx - 6, cy, cx + 6, cy);
                SDL_RenderDrawLine(renderer, cx, cy - 6, cx, cy + 6);
            } else {
                SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);
                draw_string(renderer, "ELIMINE - ECHAP POUR RETOURNER", WINDOW_WIDTH/2 - 319, WINDOW_HEIGHT/2 - 11);
            }

            if(active_bots == 0) {
                SDL_SetRenderDrawColor(renderer, 0, 255, 100, 255);
                draw_string(renderer, "VICTOIRE NETTE - TOUS MORTS", WINDOW_WIDTH/2 - 286, WINDOW_HEIGHT/2 - 11);
            }

            char soloHud[128]; sprintf(soloHud, "SOLO STEALTH | CIBLES DETRUITES : %d / 6", solo_kills);
